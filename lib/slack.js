...
export async function notifyParticipants(sb, { taskId, author, body, exceptChannel }) {
  try {
    ...
    const names = [...new Set([...ownerNames, ...commenters])].filter(n => n && n.toLowerCase() !== lc)
...
export async function notifyReview(sb, taskId, actor) {
  try {
    ...
    const masters = PEOPLE.filter(p => p.role === 'owner' || p.role === 'admin').map(p => p.name)
    const recips = masters.filter(n => n.toLowerCase() !== String(actor || '').toLowerCase())
    if (!recips.length) return { ok: true, dmed: 0 }
