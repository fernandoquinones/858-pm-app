...
const REAL_PEOPLE = ['Christina', 'Fernando', 'JG', 'Nic', 'Caitlin', 'Beth', 'Marty']
...
export async function currentProjects(sb) {
  const { data: projects } = await sb.from('projects').select('id,name,event_date,archived')
  const cur = (projects || []).filter(p => !p.archived) // "current" = not manually moved to past
...
// Tasks owned (in whole or as part of a combo) by a given person.
export function ownedBy(tasks, person) {
  return (tasks || []).filter(t => (t.owner || '').split('+').map(x => x.trim()).includes(person))
}
...
// Status labels (mirror lib/slack.js) for the per-line dropdown.
const S_LABEL = { todo: 'To do', prog: 'In progress', hold: 'On hold', ongoing: 'Ongoing', review: 'Needs review', done: 'Done' }
const S_EMOJI = { todo: '⚪️', prog: '🟡', hold: '⏸️', ongoing: '🟣', review: '🟠', done: '✅' }
function statusSelect(t) {
  const cur = S_LABEL[t.status] ? t.status : 'todo'
  const options = Object.keys(S_LABEL).map(k => ({ text: { type: 'plain_text', text: `${S_EMOJI[k]} ${S_LABEL[k]}` }, value: `${t.id}::${k}` }))
  return { type: 'static_select', action_id: 'set_status', placeholder: { type: 'plain_text', text: 'Status' }, options, initial_option: options.find(o => o.value === `${t.id}::${cur}`) }
}

// A single task rendered as a section with a status dropdown accessory.
