      ...
      {rows && (
        <div style={card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
            ...
            <div style={{ fontSize: 12, color: '#5f6672' }}>{matched}/{rows.length} matched to a contact record</div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 10, marginTop: 12 }}>
            {rows.map((r, i) => (
              <div key={i} style={{ display: 'flex', gap: 14, padding: 12, border: '1px solid #eef0f3', borderRadius: 10, alignItems: 'flex-start', background: r.matched ? '#fff' : '#fff8f6' }}>
                ...
                <select value={tier[i] || '—'} onChange={e => setTier(t => ({ ...t, [i]: e.target.value }))} style={{ border: '1px solid #e2e5ea', borderRadius: 999, padding: '5px 10px', fontFamily: 'inherit', fontSize: 12, color: '#1B2A4A', flex: 'none', alignSelf: 'center' }}>
                  {TIERS.map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
            ))}
          </div>
          <div style={{ fontSize: 11.5, color: '#9aa1ad', marginTop: 12 }}>Client picks tiers here (Tier 1/2, customer, detractor) — the only thing they edit. Unmatched rows are flagged for the team to reconcile.</div>
        </div>
      )}
    </div>
  )
